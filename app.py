...
def predict():
    ...
    if 'date' not in request.json:
        print("WARNING API (predict): received request, but no 'date' was found within")
    ...
    _result = model_predict(date=date_input, country=country_input, df=None, model=model, test=test)
    result = {}
...
def train():
    ...
    model = model_train(df=None, test=test)
...
if __name__ == '__main__':
    ...
    if args["debug"]:
        ...
        app.run(host='0.0.0.0', threaded=True, port=8081)