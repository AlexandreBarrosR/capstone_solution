...
def update_predict_log(y_pred, date_input, country_input, runtime, MODEL_VERSION, test=False):
    """
    ...
    ## write the data to a csv file    
    header = ['unique_id','timestamp','y_pred','date_input','country_input','model_version','runtime']
    write_header = False
    if not os.path.exists(logfile):
        write_header = True
    with open(logfile,'a') as csvfile:
        ...
        to_write = map(str,[uuid.uuid4(), time.time(), y_pred, date_input,country_input,
...
if __name__ == "__main__":
    ...
    ## predict logger
    update_predict_log("[0]", "2020-12-30","united_states",
